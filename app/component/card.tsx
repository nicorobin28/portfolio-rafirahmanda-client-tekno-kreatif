...
const Card: React.FC<CardProps> = ({ img, desc, title, subtitle }) => {
  return (
    <div className="flex flex-row md:flex-col gap-8 md:gap-4 w-full cursor-pointer group">
      {/* Content wrapper */}
      <div className="flex flex-col gap-2 w-full order-1 md:order-2">
        ...
        <div className="text-[#171718] font-jakarta px-[4px] group-hover:underline">
          <h1 className="font-medium text-[20px] md:text-[26px] line-clamp-2">
            {title}
          </h1>
        </div>
        ...
        <div className="text-[#5B5E61] font-jakarta px-[4px] group-hover:hidden">
          <h1 className="text-[16px] line-clamp-2">{subtitle}</h1>
        </div>
      ...
      <div className="order-2 md:order-1 shrink-0">
        {/* Desktop Image */}
        <div className="hidden md:block w-[236px] h-[162px] rounded-[8px] bg-[#F1F1F2] overflow-hidden">
          <Image src={img} alt={title} width={236} height={162} className="w-full h-full object-cover" />
        </div>
        {/* Mobile Image */}
        <div className="block md:hidden w-[108px] h-[72px] rounded-[6px] bg-[#F1F1F2] overflow-hidden">
           <Image src={img} alt={title} width={108} height={72} className="w-full h-full object-cover" />
        </div>
      </div>
    </div>
  );
